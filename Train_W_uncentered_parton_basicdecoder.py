...
import utils.VAE_model_tools_basic_decoder
from utils.VAE_model_tools_basic_decoder import build_and_compile_annealing_vae, betaVAEModel, reset_metrics, loss_tracker
...
# path to file
fn =  'E:\projects\EMD_VAE\in_data\monoW-data-parton.h5'
...
valid_x = data_x[1500000:]
valid_y = data_y[1500000:]

experiment_name = 'W-centered-parton-basicdec'
...
beta = 0.1
init_epoch=0
betas = np.concatenate((np.logspace(-2,-1,10),
                        np.flip(np.logspace(-2,-1,10))[1:],
...
i_init = 0
for i in range(i_init,len(betas)):
