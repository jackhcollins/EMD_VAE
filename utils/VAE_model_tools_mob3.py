...
#    )(layer)
    ...
    vm_z_mean = tfkl.Dense(latent_dim_vm, name='encoder_vm_z_mean',activation=None)(layer)
    vm_z_mean = tf.cast(vm_z_mean,tf.float64)
...
#     latent_inputs = tfk.Input(shape=(latent_dim_vm), name='z_sampling')
    ...
    #circle_dims = circle_dims - 2. * np.pi * tf.round(circle_dims / (2. * np.pi))

    switch_z = tf.greater(tf.math.abs(circle_dims),np.pi)
    line_dims_2 = tf.where(switch_z,-line_dims,line_dims)

    #largephi = tf.greater(circle_dims,np.pi)
    #smallphi = tf.less(circle_dims,-np.pi)
    #circle_dims_2 = tf.where(largephi, circle_dims - 2*np.pi,circle_dims)
    #circle_dims_2 = tf.where(smallphi, circle_dims + 2*np.pi,circle_dims_2)

    #circle_dims = circle_dims - 2. * np.pi * tf.round(circle_dims / (2. * np.pi))
    
    #circle_dims = circle_dims + np.pi
    #circle_dims = tf.math.floormod(circle_dims,np.pi*2)
    #circle_dims = circle_dims - np.pi
    
    circle_x = tf.sin(circle_dims)
    circle_y = tf.cos(circle_dims)
    circle_dims_2 = tf.atan2(circle_x,circle_y)

    layer = tfkl.Concatenate()([line_dims_2,circle_dims_2])
