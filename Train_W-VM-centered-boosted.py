...
from utils.VAE_model_tools_vm import build_and_compile_annealing_vae, betaVAEModel, reset_metrics, myTerminateOnNaN, loss_tracker
...
betas = np.zeros(0)
for i in range(6):
  betas = np.append(betas, np.flip(beta_set[i:i+11:1]))
  betas = np.append(betas, beta_set[i+2:i+11:1])
for i in range(3,10):
  betas = np.append(betas, np.flip(beta_set[2*i:2*i + 11:2]))
  betas = np.append(betas, beta_set[2*i+2:2*i+11:2])

last_run_i = len(betas)
betas = np.append(betas, np.flip(beta_set))
