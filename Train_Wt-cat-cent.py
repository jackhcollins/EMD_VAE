...
if args.parton:
  ...
  fnW =  args.data_path + '/monoW-data-3.h5'
  fnt =  args.data_path + '/t-data.h5'
  numparts = 50
  print("Using particle data")
  numtrain = 1000000

print("Loading ", fnW)
dfW = pandas.read_hdf(fnW,stop=600000)
print(dfW.shape)
print("Memory in GB:",sum(dfW.memory_usage(deep=True)) / (1024**3)+sum(dfW.memory_usage(deep=True)) / (1024**3))

print("Loading ", fnt)
dft = pandas.read_hdf(fnt,stop=600000)
print(dft.shape)
print("Memory in GB:",sum(dft.memory_usage(deep=True)) / (1024**3)+sum(dft.memory_usage(deep=True)) / (1024**3))

# Data file contains, for each event, 50 particles (with zero padding), each particle with pT, eta, phi, E.
data = np.concatenate((dfW.values,dft.values)).reshape((-1,numparts,4))
